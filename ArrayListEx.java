...
public class ArrayListEx {
	...
	public static void main(String[] args) {
		...
		while(true) {
			System.out.println("탑 100 입니다. 가수 or 제목 어떤 걸로 알려 드릴까요?(종료:q)");
			System.out.print(">>>");
			String selct = scan.nextLine();
			if(selct.equalsIgnoreCase("q")) {  // equalsIgnoreCase 대소문자 상관없이 같은지
			...
			ArrayList<String> result = searchSong(selct, keyword, playList);
			...
			if(result.isEmpty()) {
				...
				// 건수는 (size())
				System.out.println("검색 결과는 총:" + result.size()+ " 전");
	...
	public static ArrayList<String> searchSong(String option
		...
		// 전달 받은 option을 기준으로 keyword 포함된 데이터를 찾아서 리턴
